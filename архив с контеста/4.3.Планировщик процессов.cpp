...
public:
    ...
    std::size_t size() { return _tail; }

    T& operator[](std::size_t n) { return _buffer[n]; }
...
int run(std::istream& input, std::ostream& output) {
    std::size_t n = 0;
    input >> n;
    Heap<Process, decltype(isLessProcess)*> heap(isLessProcess);
    Process process;
    for (std::size_t i = 0; i < n; ++i) {
        input >> process._P >> process._T;
        process.updateWeight();
        heap.insert(process);  // Метод сразу помещает элемент в нужное место в куче
    }
